...
template <char C> constexpr bool isChar(char ch) { return ch == C; }
constexpr bool isDigit(char ch) { return ch >= '0' && ch <= '9'; }
constexpr bool isNotDigit(char ch) { return !isDigit(ch); }
constexpr bool isWord(char ch) {
  return (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') ||
         (ch >= '0' && ch <= '9') || ch == '_';
}
constexpr bool isNotWord(char ch) { return !isWord(ch); }
constexpr bool isSpace(char ch) {
  return ch == ' ' || ch == '\t' || ch == '\n' || ch == '\r' || ch == '\f' ||
         ch == '\v';
}
constexpr bool isNotSpace(char ch) { return !isSpace(ch); }
constexpr bool isAnyChar(char) { return true; }
constexpr bool isDecDigit(char ch) { return ch >= '0' && ch <= '9'; }
} // namespace CharHelper
...
template <FixedString Pattern, bool Counting, typename NameResolver,
          std::size_t MaxNodes = 0>
class ConstexprRegexParser {
  static constexpr std::size_t kPatternSize = Pattern.size();
  static constexpr std::size_t kTreeNodes = MaxNodes == 0 ? 1 : MaxNodes;
  static constexpr std::size_t kMaxNameLen =
      kPatternSize == 0 ? 1 : kPatternSize;

public:
  using ParseResult = std::conditional_t<Counting, NodeCountResult,
                                         regex::RegexTree<kTreeNodes>>;
...
private:
  ...
  [[nodiscard]] consteval bool eof() const { return pos_ >= kPatternSize; }
  ...
  consteval auto makeBinaryNode(regex::RegexTreeNodeKind kind, int lhs, int rhs)
      -> int {
  ...
  consteval auto makeRepetitionNode(int child, int minRepeat, int maxRepeat)
      -> int {
  ...
  consteval auto parseEscape() -> int {
    ...
    case 'd':
      return makeCharCheckNode(CharHelper::isDigit);
    case 'D':
      return makeCharCheckNode(CharHelper::isNotDigit);
    case 'w':
      return makeCharCheckNode(CharHelper::isWord);
    case 'W':
      return makeCharCheckNode(CharHelper::isNotWord);
    case 's':
      return makeCharCheckNode(CharHelper::isSpace);
    case 'S':
      return makeCharCheckNode(CharHelper::isNotSpace);
    case 't':
      return makeLiteralNode('\t');
    case 'n':
      return makeLiteralNode('\n');
    case 'r':
      return makeLiteralNode('\r');
    case 'f':
      return makeLiteralNode('\f');
    case 'v':
      return makeLiteralNode('\v');
    case '0':
      return makeLiteralNode('\0');
    case 'g':
      return parseTagEscape();
    case 'x':
      return parseNamedCharCheckEscape();
    default:
      return makeLiteralNode(esc);
    }
  }
...
consteval auto parseToRegexTree(NameResolver resolver) {
  using Counter =
      detail::ConstexprRegexParser<Pattern, true, detail::NoopResolver>;
  constexpr auto count = Counter(detail::NoopResolver{}).parse();
  constexpr std::size_t exactNodes = count.ok() ? count.nodeCount : 1;
  using Parser =
      detail::ConstexprRegexParser<Pattern, false, std::decay_t<NameResolver>,
                                   exactNodes>;
...
template <FixedString Pattern> consteval auto parseToRegexTree() {
...
template <FixedString Pattern> consteval auto parseToEx() {
