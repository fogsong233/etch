...
struct Regex2PrintNode : public regex::RecursiveRegexVisitor {
  ...
  bool visitInvalidNode(int nodeIdx) final {
    const auto idx =
        newPrintNode("invalid node in index " + std::to_string(nodeIdx));
    addChild(parent, idx);
    return true;
  }
  ...
  bool visitCharCheck(int nodeIdx, CharFn checkFn) final {
    const auto idx =
        newPrintNode("char check@" +
                     std::to_string(reinterpret_cast<std::uintptr_t>(checkFn)));
    addChild(parent, idx);
    return true;
  }
  ...
  bool visitRepetition(int nodeIdx, int child, int minRepeat,
                       int maxRepeat) final {
    const auto idx =
        newPrintNode("repetition {" + std::to_string(minRepeat) + "," +
  ...
  void print(std::ostream &o) const {
    if (tree.ok()) {
      ...
      o << "Failed to parse regex tree: "
        << regex::RegexParseErrorToString(tree.error) << " at position "
        << tree.errorPos << '\n';
    }
  }
};
